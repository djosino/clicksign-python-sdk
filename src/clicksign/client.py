...
class Client(RequestInstrumentation):
    ...
    def delete(
        ...
        *,
        ...
        return self._request("DELETE", path, options=options)  # type: ignore[no-any-return]
    ...
    def deserialize(
        ...
        body = response.body if isinstance(response, RawResponse) else response
        if not isinstance(body, dict):
            raise TypeError("deserialize expects a JSON object body")

        instances, _ = resource_class._parse_response(body)
        metadata = response.metadata if isinstance(response, RawResponse) else None
        resource_class._attach_last_response(instances, metadata)
